...
class Kernel
{
    ...
    static function boot() {
        ...
        self::$environment->set("time", "Load of class & Define env:" .($classTime = self::$environment->getExecutionTime()), true);
        self::$annotation = new Annotation();
        self::$environment->set("time", "AnnotationInit:" .($annoTime = self::$environment->getExecutionTime() - $classTime), true);
        self::$routing = new Routing();
        self::$environment->set("time", "RoutingInit:" .($routingTime = self::$environment->getExecutionTime() - $annoTime), true);
        Logger::log("general", "KERNEL|Initialize", Logger::$DEFAULT_LEVEL);
        Event::addEventByAnnotation();
        self::$environment->set("time", "EventInit:" .($eventTime = self::$environment->getExecutionTime() - $routingTime), true);
        ...
        self::$environment->set("time", "ControllerCall:" .($controllerTime = self::$environment->getExecutionTime() - $routingTime), true);
    }
